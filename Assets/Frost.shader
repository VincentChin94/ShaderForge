...
////// Specular:
                ...
                float3 node_8227 = max(_Diffuse_var.rgb,lerp((node_3299_if_leA*node_7414)+(node_3299_if_leB*abs(dot(node_2641.g,node_1323))),node_7414,node_3299_if_leA*node_3299_if_leB));
                float3 specularColor = node_8227;
                float3 directSpecular = attenColor * pow(max(0,dot(halfDirection,normalDirection)),specPow)*specularColor;
                float3 specular = directSpecular;
/////// Diffuse:
                NdotL = dot( normalDirection, lightDirection );
                float3 forwardLight = max(0.0, NdotL );
                float3 backLight = max(0.0, -NdotL ) * node_8227;
                NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = (forwardLight+backLight) * attenColor;
                float3 indirectDiffuse = float3(0,0,0);
                indirectDiffuse += UNITY_LIGHTMODEL_AMBIENT.rgb; // Ambient Light
                float3 diffuseColor = node_8227;
